function [TrialData] = mainWorkPowerAnalysisMW(TrialData,subj)
    ...
    %   filtered and processed a great deal (median filter, gap fill, 
    %   lowpass filter) already through procbatch.
        ...
        if any(strcmpi(TrialData(n).Info.Condition, to_process))
            ...
            if any(strcmpi(TrialData(n).Info.Condition, assisted))
                ...
                if isfield(Markers,'FH')
                    FH = TrialData(n).Markers.FH; % Tolerance check in later recoverForces code looks at mm so don't convert to m
...
%                     
    ...
    %                 temp.swayX = clavFilt(1,:) - nanmean(clavFilt(1,:));
                    ...
                    TrialData(n).Results.RASI = Markers.POB.RASI(start_idx:stop_idx,2)./1000; % (m)

                    % Store velocity for later analysis/plotting
                    TrialData(n).Results.vyRASI = diff(TrialData(n).Results.RASI).*fs; % (m/s)
