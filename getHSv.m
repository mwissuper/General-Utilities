function indHS = getHSv(markerPos,B,A,order,subj,trial,side)
...
% Updated with comments for filtering in Matlab

vThresh = 0.002;
    
 if isempty(find(isnan(diff(markerPos)),1,'first')) % no nan values, ok to use filtfilt
    v = filtfilt(B,A,diff(markerPos));
else
    v = filtIgnoreNan(B,A,order,diff(markerPos));
 end
x = 1:length(v);
